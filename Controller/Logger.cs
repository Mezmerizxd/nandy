...
namespace Controller
{
    internal class Logger
    {
        ...
        public static void Initialize(Dashboard? d)
        {
            if (d != null)
            {
                dashboard = d;
        ...
        public static void Log(string message, TYPE type)
        {
            if (dashboard == null)
            {
                Console.WriteLine("Failed to find dashboard");
                return;
            }
            switch (type)
            {
                case TYPE.INFO:
                    ...
                    dashboard.LogToConsole("[INFO] " + message);
                    break;
                case TYPE.WARNING:
                    Console.ForegroundColor = ConsoleColor.Yellow;
                    Console.WriteLine("[WARNING] " + message);
                    dashboard.LogToConsole("[WARNING] " + message);
                    break;
                case TYPE.ERROR:
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("[ERROR] " + message);
                    dashboard.LogToConsole("[ERROR] " + message);
                    break;
            }
        }
    }
